...
// Static files
// noCache: HTML/JS pages must never be cached by the browser, otherwise
// the user keeps seeing a stale build after we ship fixes.
const noCache = {
  etag: false,
  lastModified: false,
  setHeaders: (res) => {
...
app.use('/admin',   express.static(path.join(__dirname, '..', 'public', 'admin'),  noCache));
app.use('/gate',    express.static(path.join(__dirname, '..', 'public', 'gate'),   noCache));
app.use('/login',   express.static(path.join(__dirname, '..', 'public', 'login'),  noCache));
app.use('/teacher', express.static(path.join(__dirname, '..', 'public', 'teacher'), noCache));
app.use('/shared',  express.static(path.join(__dirname, '..', 'public', 'shared'), noCache));
...
app.get('/api/dashboard/fee-chart', async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      SELECT payment_month::int                  AS month,
             COALESCE(SUM(amount), 0)::float     AS total
      FROM fee_payments
      WHERE ($1::text IS NULL OR payment_year = $1)
        AND payment_month ~ '^[0-9]+$'
      GROUP BY payment_month::int
      ORDER BY month
    `, [year]);
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
